...
| **Feature View Descriptor** | A Feature's single view-side registration entry in the shell's `feature-views.ts` list — display identity (name, icon, description), Widgets with default grid sizes, optional Full View with named modal size and focus-mode capability, nav keywords, optional Provider, and optional Command Registrar — from which the shell derives all per-feature rendering and Commands | Feature view config, view manifest, FEATURE_META (retired) |
...
- The shell lists one **Feature View Descriptor** per **Feature** in `feature-views.ts`; from it the shell renders the Feature's **Widgets**, Full View modal, Focus Mode content, and Feature Catalog entry, folds in its Provider, generates its navigation and focus-mode **Commands**, and mounts its **Command Registrar** (if any) to register the rest.
