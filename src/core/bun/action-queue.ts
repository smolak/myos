...
export class ActionQueue {
	...
	async processExecution(executionId: string): Promise<void> {
		...
		this.db
			.query(
				"UPDATE execution_actions SET status = 'pending' WHERE execution_id = ? AND status = 'running'",
			)
			.run(executionId);
	...
	private async executeAction(action: ActionRow, outputCache: Map<string, unknown>): Promise<void> {
		...
		this.db
			.query("UPDATE execution_actions SET status = 'running' WHERE id = ?")
			.run(action.id);
		...
		while (true) {
			...
			try {
				...
				this.db
					.query("UPDATE execution_actions SET status = 'pending' WHERE id = ?")
					.run(action.id);
	...
	private completeAction(id: string, result: unknown): void {
		this.db
			.query(
				"UPDATE execution_actions SET status = 'completed', result = ?, completed_at = ? WHERE id = ?",
			)
	...
	private markFailed(id: string, error: string): void {
		this.db
			.query(
				"UPDATE execution_actions SET status = 'failed', error = ?, completed_at = ? WHERE id = ?",
			)
