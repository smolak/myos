...
export function getAppliedMigrations(db: Database, featureId: string): string[] {
  ...
  const rows = db.query("SELECT version FROM migrations WHERE feature_id = ? ORDER BY version ASC").all(featureId) as {
    version: string;
  }[];
...
export function runMigrations(
  ...
  for (const migration of migrations) {
    ...
    const runOne = db.transaction(() => {
      ...
      db.run("INSERT INTO migrations (feature_id, version, name, applied_at) VALUES (?, ?, ?, ?)", [
        featureId,
        migration.version,
        migration.name,
        appliedAt,
