...
function getAction(db: Database, executionId: string, sequence: number) {
  return db
    .query<{ status: string; result: string | null; error: string | null; retry_count: number }, [string, number]>(
...
describe("ActionQueue", () => {
  ...
  describe("enqueue", () => {
    ...
    test("persists multiple actions in a single transaction", () => {
      ...
      const count = db.query<{ n: number }, []>("SELECT COUNT(*) as n FROM execution_actions").get();
    ...
    test("stores optional correlationId", () => {
      ...
      const row = db.query<{ correlation_id: string | null }, []>("SELECT correlation_id FROM execution_actions").get();
    ...
    test("uses default max_retries when not provided", () => {
      seedExecution(db, "exec-1");
      queue.enqueue("exec-1", [{ sequence: 1, featureId: "todo", actionName: "create", params: {} }]);

      const row = db.query<{ max_retries: number }, []>("SELECT max_retries FROM execution_actions").get();
  ...
  describe("processExecution", () => {
    ...
    test("passes correct params to handler", async () => {
      ...
      queue.enqueue("exec-1", [{ sequence: 1, featureId: "todo", actionName: "create", params: { title: "Hello" } }]);
    ...
    test("sets completed_at when action succeeds", async () => {
      ...
      const row = db.query<{ completed_at: string | null }, []>("SELECT completed_at FROM execution_actions").get();
      expect(row?.completed_at).toMatch(/^\d{4}-\d{2}-\d{2}T/);
    });

    test("marks action failed when no handler is registered", async () => {
      seedExecution(db, "exec-1");
      queue.enqueue("exec-1", [{ sequence: 1, featureId: "todo", actionName: "missing", params: {} }]);
    ...
    describe("retry", () => {
      test("retries failed action and succeeds on subsequent attempt", async () => {
        ...
        queue.enqueue("exec-1", [{ sequence: 1, featureId: "todo", actionName: "create", params: {}, maxRetries: 3 }]);
      ...
      test("marks action failed after exhausting max retries", async () => {
        ...
        queue.enqueue("exec-1", [{ sequence: 1, featureId: "todo", actionName: "create", params: {}, maxRetries: 2 }]);
