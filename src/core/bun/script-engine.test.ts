...
function insertScript(db: Database, id: string, name: string, code: string, enabled = 1): void {
	...
	db.query(
		"INSERT INTO scripts (id, name, code, enabled, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)",
	).run(id, name, code, enabled, now, now);
}
...
function getExecution(
	db: Database,
	scriptId: string,
): { status: string; triggered_by: string } | null {
	return db
...
describe("ScriptEngine", () => {
	...
	describe("ctx.on", () => {
		...
		test("handler is not called for a different event", async () => {
			insertScript(
				...
				"s1",
				"listener",
				`ctx.on("todo:created", async () => { await ctx.store.set("ran", true); });`,
			);
	...
	describe("ctx.store", () => {
        ...
        `,
			...
			db.query(
				"INSERT INTO script_store (script_id, key, value, updated_at) VALUES (?, ?, ?, ?)",
			).run("s1", "val", JSON.stringify("first"), new Date().toISOString());
	...
	describe("script_executions tracking", () => {
		...
		test("marks execution as failed when handler throws", async () => {
			insertScript(
				...
				"s1",
				"fail-test",
				`ctx.on("test:event", async () => { throw new Error("boom"); });`,
			);
