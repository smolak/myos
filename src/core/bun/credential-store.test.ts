...
describe("CredentialStore", () => {
	...
	beforeEach(async () => {
		...
		coreDb.query("INSERT INTO features (id, name, version, enabled, manifest, installed_at, updated_at) VALUES ('weather', 'Weather', '1.0.0', 1, '{}', datetime('now'), datetime('now'))").run();
	...
	describe("store and retrieve", () => {
		...
		test("overwrites existing credential on store", async () => {
			...
			const count = dbManager
				.getCoreDatabase()
				.query<{ c: number }, []>("SELECT COUNT(*) as c FROM credentials")
				.get();
	...
	describe("forScope", () => {
		...
		test("scoped accessor is isolated from other feature scopes", async () => {
			const coreDb = dbManager.getCoreDatabase();
			coreDb.query("INSERT INTO features (id, name, version, enabled, manifest, installed_at, updated_at) VALUES ('todo', 'Todo', '1.0.0', 1, '{}', datetime('now'), datetime('now'))").run();
