...
export class Scheduler {
  ...
  registerTask(input: RegisterTaskInput): void {
    const { taskId, featureId, name, scheduleType, scheduleValue, maxRetries = DEFAULT_MAX_RETRIES } = input;
  ...
  private calculateNextRun(scheduleType: "cron" | "interval", scheduleValue: string | number): Date {
    if (scheduleType === "interval") {
      const ms = typeof scheduleValue === "number" ? scheduleValue : parseInt(String(scheduleValue), 10);
