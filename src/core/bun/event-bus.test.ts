...
describe("EventBus", () => {
  ...
  describe("emit", () => {
    test("logs event to event_log table", () => {
      ...
      const row = db
        .query<{ event_name: string; feature_id: string; payload: string | null }, []>(
    ...
    test("logs null payload when payload is undefined", () => {
      ...
      const row = db.query<{ payload: string | null }, []>("SELECT payload FROM event_log").get();
    ...
    test("stores created_at as ISO 8601 string", () => {
      ...
      const row = db.query<{ created_at: string }, []>("SELECT created_at FROM event_log").get();
    ...
    test("emitting with no subscribers still logs the event", () => {
      ...
      const count = db.query<{ n: number }, []>("SELECT COUNT(*) as n FROM event_log").get();
