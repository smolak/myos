...
export class CredentialStore {
	...
	async delete(featureId: string, serviceName: string, credentialType?: string): Promise<void> {
		if (credentialType !== undefined) {
			this.db
				.query(
					"DELETE FROM credentials WHERE feature_id = ? AND service_name = ? AND credential_type = ?",
				)
				.run(featureId, serviceName, credentialType);
		} else {
			this.db
				.query("DELETE FROM credentials WHERE feature_id = ? AND service_name = ?")
	...
	forScope(featureId: string): ScopedCredentials {
		return {
			store: (serviceName, credentialType, value) =>
				this.store(featureId, serviceName, credentialType, value),
			retrieve: (serviceName, credentialType) =>
				this.retrieve(featureId, serviceName, credentialType),
			delete: (serviceName, credentialType) =>
				this.delete(featureId, serviceName, credentialType),
		};
	}
}
