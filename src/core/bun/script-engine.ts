...
export class ScriptEngine {
	...
	start(): void {
		const scripts = this.coreDb
			.query<ScriptRow, []>("SELECT id, name, code FROM scripts WHERE enabled = 1")
			.all();
	...
	private initializeScript(script: ScriptRow): void {
		...
		for (const { event, handler } of registeredHandlers) {
			this.eventBus.subscribe(event, (payload) =>
				this.runHandler(script.id, script.name, event, payload, handler),
			);
		}
	}
	...
	private async runHandler(
		...
		try {
			...
			this.coreDb
				.query(
					"UPDATE script_executions SET status = 'completed', completed_at = ? WHERE id = ?",
				)
				.run(new Date().toISOString(), executionId);
		} catch (err) {
			console.error(
				`[script-engine] Script "${scriptName}" handler failed for "${eventName}":`,
				err,
			);
			this.coreDb
				.query(
					"UPDATE script_executions SET status = 'failed', completed_at = ? WHERE id = ?",
				)
				.run(new Date().toISOString(), executionId);
		}
	}

	private buildActionsProxy(): Record<
		string,
				...
				return new Proxy({} as Record<string, (params: unknown) => Promise<unknown>>, {
					get(_target, actionName: string) {
						return async (params: unknown): Promise<unknown> => {
							...
							if (!exec)
								throw new Error("ctx.actions called outside of script execution context");
							...
							const row = coreDb
								.query<
									{ result: string | null; status: string; error: string | null },
									[string, number]
								>(
							...
							if (row?.status === "failed") {
								throw new Error(
									row.error ?? `Action ${featureId}:${actionName} failed`,
								);
							}
