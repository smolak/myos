...
export class FeatureRegistry {
  ...
  private async register(feature: FeatureDefinition): Promise<void> {
    ...
    const existing = this.coreDb
      .query<{ id: string; version: string }, [string]>("SELECT id, version FROM features WHERE id = ?")
      .get(feature.id);
  ...
  private async activate(feature: FeatureDefinition): Promise<void> {
    ...
    try {
      ...
      const now = new Date().toISOString();
      this.coreDb.query("UPDATE features SET enabled = 0, updated_at = ? WHERE id = ?").run(now, feature.id);
    ...
    return {
      ...
      actions: {
        handle<K extends keyof ActionMap>(
          action: K,
          handler: (params: ActionMap[K]["params"], meta: ActionMeta) => Promise<ActionMap[K]["result"]>,
        ): void {
          actionQueue.registerHandler(
            featureId,
            action as string,
            handler as Parameters<typeof actionQueue.registerHandler>[2],
          );
        },
      },
      queries: {
        handle<K extends keyof QueryMap>(
          ...
          actionQueue.registerQueryHandler(
            featureId,
            query as string,
            handler as Parameters<typeof actionQueue.registerQueryHandler>[2],
          );
        },
      },
