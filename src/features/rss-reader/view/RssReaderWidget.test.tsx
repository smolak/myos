...
vi.mock("./useRssReader", () => {
	...
	return {
		...
		__setFeeds: (f: typeof feeds) => { feeds = f; },
		__setEntries: (e: typeof entries) => { entries = e; },
		__setIsLoading: (l: boolean) => { isLoading = l; },
...
function makeFeed() {
	return { id: "f1", url: "https://example.com/feed", title: "Test Feed", fetchIntervalMinutes: 30, lastFetchedAt: null };
}
	...
	test("shows at most 5 entries", () => {
		...
		setEntries(
			Array.from({ length: 7 }, (_, i) =>
				makeEntry({ id: `e${i}`, title: `Article ${i}` }),
			),
		);
		render(<RssReaderWidget />);
