...
export const rssReaderFeature: FeatureDefinition<
	RssReaderEvents,
	RssReaderActions,
	RssReaderQueries
> = {
	...
	manifest: {
		...
		permissions: [
			{ type: "network", reason: "Fetch RSS feeds from the internet" },
		],
		scheduledTasks: [
			{
	...
	async activate(ctx) {
		...
		ctx.actions.handle("mark-read", async (params, _meta) => {
			const entryRow = ctx.db
				.query<{ feed_id: string }, [string]>(
					"SELECT feed_id FROM rss_entries WHERE id = ?",
				)
				.get(params.id);
		...
		ctx.scheduler.register(
			"rss-reader:fetch-feeds",
			{ type: "interval", value: FETCH_INTERVAL_MS },
			async () => {
				const { fetched, newEntries } = await fetchAllFeeds(ctx.db);
				for (const entry of newEntries) {
					ctx.events.emit("rss:new-entry", {
