...
import { addFeed, deleteFeed, markRead, markUnread } from "./actions";
...
export function createRssReaderFeature(
  fetchFn: FetchFn = fetch,
): FeatureDefinition<RssReaderEvents, RssReaderActions, RssReaderQueries> {
  return {
    id: "rss-reader",
    name: "RSS Reader",
    version: "1.0.0",
    ...
    manifest: {
      ...
      queries: {
        ...
        "get-favicons": {
          description: "Get all cached favicons as a hostname → data: URL map",
          params: {},
          result: "Record<hostname, dataUrl>",
        },
        search: {
    ...
    async activate(ctx) {
      ...
      ctx.actions.handle("fetch-feeds", async (_params, _meta) => {
        const { fetched, newEntries } = await ingestFeeds(ctx, fetchFn);
      ...
      ctx.queries.handle("get-favicons", async (params) => {
        return getFavicons(ctx.db, params);
      });
