...
describe("rssReaderFeature definition", () => {
	...
	test("manifest declares scheduled fetch task", () => {
		const task = rssReaderFeature.manifest.scheduledTasks.find(
			(t) => t.id === "rss-reader:fetch-feeds",
		);
...
describe("cross-feature: rss:new-entry → todo:create", () => {
	...
	test("script creates a todo when a new RSS entry arrives", async () => {
		...
		coreDb
			.query(
				"INSERT INTO scripts (id, name, code, enabled, created_at, updated_at) VALUES (?, ?, ?, 1, ?, ?)",
			)
			.run(
				"rss-to-todo",
				"RSS to Todo",
