...
): Promise<RssReaderActions["add-feed"]["result"]> {
	const existing = db
		.query<{ id: string }, [string]>("SELECT id FROM rss_feeds WHERE url = ?")
		.get(params.url);
...
export async function fetchAllFeeds(
	...
	const feeds = db
		.query<{ id: string; url: string }, []>(
			"SELECT id, url FROM rss_feeds ORDER BY created_at ASC",
		)
		.all();
	...
	for (const feed of feeds) {
		try {
			...
			const now = new Date().toISOString();
			db.query(
				"UPDATE rss_feeds SET title = ?, last_fetched_at = ?, updated_at = ? WHERE id = ?",
			).run(parsed.title, now, now, feed.id);
...
): Promise<RssReaderActions["mark-read"]["result"]> {
	const row = db
		.query<{ id: string }, [string]>("SELECT id FROM rss_entries WHERE id = ?")
		.get(params.id);
	if (!row) return { success: false };
