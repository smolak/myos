...
describe("addFeed", () => {
	...
	test("stores the feed in the database", async () => {
		...
		const row = db
			.query<{ url: string; title: string }, [string]>(
				"SELECT url, title FROM rss_feeds WHERE id = ?",
			)
			.get(id);
	...
	test("falls back to URL as title when title not provided", async () => {
		...
		const row = db
			.query<{ title: string }, [string]>("SELECT title FROM rss_feeds WHERE id = ?")
			.get(id);
	...
	test("defaults fetchIntervalMinutes to 30", async () => {
		...
		const row = db
			.query<{ fetch_interval_minutes: number }, [string]>(
				"SELECT fetch_interval_minutes FROM rss_feeds WHERE id = ?",
			)
			.get(id);
...
describe("deleteFeed", () => {
	...
	test("removes the feed from the database", async () => {
		...
		const row = db
			.query<{ id: string }, [string]>("SELECT id FROM rss_feeds WHERE id = ?")
			.get(id);
		expect(row).toBeNull();
	});
...
describe("fetchAllFeeds", () => {
	...
	test("fetches entries and stores them in the database", async () => {
		...
		const entries = db
			.query<{ id: string }, [string]>("SELECT id FROM rss_entries WHERE feed_id = ?")
			.all(feedId);
	...
	test("sets last_fetched_at after fetching", async () => {
		...
		const row = db
			.query<{ last_fetched_at: string | null }, [string]>(
				"SELECT last_fetched_at FROM rss_feeds WHERE id = ?",
			)
			.get(feedId);
	...
	test("new entries have is_read=0 by default", async () => {
		...
		await fetchAllFeeds(db, makeFetchFn(RSS_XML));
		const entries = db
			.query<{ is_read: number }, []>("SELECT is_read FROM rss_entries")
			.all();
...
describe("markRead / markUnread", () => {
	...
	test("markRead sets is_read=1", async () => {
		...
		const row = db
			.query<{ is_read: number }, [string]>("SELECT is_read FROM rss_entries WHERE id = ?")
			.get(entryId);
