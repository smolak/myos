...
export async function getFeeds(
	...
	return db
		.query<FeedRow, []>("SELECT * FROM rss_feeds ORDER BY created_at ASC")
		.all()
		.map(rowToFeed);
}
...
export async function getEntries(
	...
	if (params.feedId) {
		return db
			.query<EntryRow, [string, number]>(
				"SELECT * FROM rss_entries WHERE feed_id = ? ORDER BY created_at DESC LIMIT ?",
			)
	...
	if (params.unreadOnly) {
		return db
			.query<EntryRow, [number, number]>(
				"SELECT * FROM rss_entries WHERE is_read = ? ORDER BY created_at DESC LIMIT ?",
			)
			.all(0, limit)
			.map(rowToEntry);
	}
	...
	return db
		.query<EntryRow, [number]>(
			"SELECT * FROM rss_entries ORDER BY created_at DESC LIMIT ?",
		)
		.all(limit)
		.map(rowToEntry);
}
...
export async function getUnreadCount(
	...
	const row = db
		.query<{ count: number }, []>(
			"SELECT COUNT(*) AS count FROM rss_entries WHERE is_read = 0",
		)
		.get();
