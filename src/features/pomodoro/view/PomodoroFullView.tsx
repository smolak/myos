...
function SessionHistoryItem({ session }: { session: PomodoroSession }) {
  ...
  return (
    <li className="flex items-center justify-between py-2 border-b border-zinc-800 last:border-0">
      <div>
        <span className={`text-xs font-medium ${session.type === "work" ? "text-zinc-200" : "text-zinc-400"}`}>
...
export function PomodoroFullView({ onClose }: Props) {
  const { session, settings, remaining, start, pause, resume, complete, cancel, updateSettings } = usePomodoro();
        ...
        {activeTab === "timer" && (
          <div className="flex flex-col items-center gap-6">
            ...
            <div className="text-center">
              ...
              {isDone && <p className="text-green-500 text-sm mt-2">Session complete! Great work.</p>}
              {isPaused && <p className="text-zinc-500 text-sm mt-2">Paused</p>}
            </div>
...
function SessionHistoryTab() {
  ...
  const storedRaw = typeof localStorage !== "undefined" ? localStorage.getItem("pomodoro:state") : null;
  ...
  return (
    <div>
      <h3 className="text-xs font-medium text-zinc-500 uppercase tracking-wide mb-3">Completed Sessions</h3>
...
function SettingsTab({ workMinutes, breakMinutes, onUpdate }: SettingsTabProps) {
  ...
  return (
    <div className="max-w-sm space-y-4">
      <h3 className="text-xs font-medium text-zinc-500 uppercase tracking-wide mb-3">Timer Settings</h3>
