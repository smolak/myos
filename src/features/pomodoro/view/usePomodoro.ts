...
export function usePomodoro(): UsePomodoroReturn {
  ...
  useEffect(() => {
    void Promise.all([rpc.request["pomodoro:get-current"]({}), rpc.request["pomodoro:get-settings"]({})]).then(
      ([currentSession, savedSettings]) => {
        if (currentSession) {
  ...
  const start = useCallback(
    async (type: SessionType = "work") => {
      const durationSeconds = type === "work" ? settings.workDurationMinutes * 60 : settings.breakDurationMinutes * 60;
  ...
  const remaining = session ? Math.max(0, session.durationSeconds - session.elapsedSeconds) : 0;
