...
export async function startSession(db: Database, params: StartParams): Promise<PomodoroActions["start"]["result"]> {
  const type = params.type ?? "work";
  const durationSeconds = params.durationSeconds ?? (type === "work" ? DEFAULT_WORK_SECONDS : DEFAULT_BREAK_SECONDS);
...
export async function pauseSession(db: Database, params: PauseParams): Promise<PomodoroActions["pause"]["result"]> {
  const row = db
...
export async function resumeSession(db: Database, params: ResumeParams): Promise<PomodoroActions["resume"]["result"]> {
  const row = db
    .query<{ id: string; status: string }, [string]>("SELECT id, status FROM pomodoro_sessions WHERE id = ?")
    .get(params.id);
...
export async function completeSession(
  ...
  if (params.elapsedSeconds !== undefined) {
    ...
    db.query("UPDATE pomodoro_sessions SET status = 'completed', ended_at = ?, updated_at = ? WHERE id = ?").run(
      ...updates,
    );
  }
  return { success: true };
...
export async function cancelSession(db: Database, params: CancelParams): Promise<PomodoroActions["cancel"]["result"]> {
  const row = db
    .query<{ id: string; status: string }, [string]>("SELECT id, status FROM pomodoro_sessions WHERE id = ?")
    .get(params.id);
  if (!row) return { success: false };
  ...
  const now = new Date().toISOString();
  db.query("UPDATE pomodoro_sessions SET status = 'cancelled', ended_at = ?, updated_at = ? WHERE id = ?").run(
    now,
    now,
    params.id,
  );
