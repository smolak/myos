...
describe("Pomodoro actions", () => {
	...
	describe("startSession", () => {
		...
		test("defaults type to 'work'", async () => {
			...
			const row = db
				.query<{ type: string }, [string]>(
					"SELECT type FROM pomodoro_sessions WHERE id = ?",
				)
				.get(id);
		...
		test("defaults work duration to 25 minutes", async () => {
			...
			const row = db
				.query<{ duration_seconds: number }, [string]>(
		...
		test("cancels any existing running session before starting new one", async () => {
			...
			const row = db
				.query<{ status: string }, [string]>(
					"SELECT status FROM pomodoro_sessions WHERE id = ?",
				)
				.get(first);
	...
	describe("pauseSession", () => {
		...
		test("is idempotent on elapsed_seconds — pausing twice preserves last value", async () => {
			...
			const row = db
				.query<{ elapsed_seconds: number }, [string]>(
					"SELECT elapsed_seconds FROM pomodoro_sessions WHERE id = ?",
				)
				.get(id);
		...
		test("sets ended_at to a valid ISO timestamp", async () => {
			...
			const row = db
				.query<{ ended_at: string | null }, [string]>(
					"SELECT ended_at FROM pomodoro_sessions WHERE id = ?",
				)
				.get(id);
