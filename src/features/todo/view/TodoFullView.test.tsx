...
function makeTodo(overrides: Partial<{ id: string; title: string; completed: boolean; completedAt: string | null }> = {}) {
	return {
		id: overrides.id ?? "t1",
...
describe("TodoFullView", () => {
	...
	test("renders completed todos in the Completed section", () => {
		currentTodos = [
			makeTodo({ id: "t1", title: "Done task", completed: true, completedAt: new Date().toISOString() }),
		];
		render(<TodoFullView />);
