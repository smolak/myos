...
export async function createTodo(
	db: Database,
	params: CreateParams,
): Promise<TodoActions["create"]["result"]> {
	const id = nanoid();
...
export async function updateTodo(
	db: Database,
	params: UpdateParams,
): Promise<TodoActions["update"]["result"]> {
	const row = db
...
): Promise<TodoActions["complete"]["result"]> {
	const row = db
		.query<{ id: string }, [string]>("SELECT id FROM todos WHERE id = ?")
		.get(params.id);
	if (!row) return { success: false };

	const now = new Date().toISOString();
	db.query("UPDATE todos SET completed = 1, completed_at = ?, updated_at = ? WHERE id = ?").run(
		now,
		now,
		params.id,
	);
...
export async function deleteTodo(
	db: Database,
	params: DeleteParams,
): Promise<TodoActions["delete"]["result"]> {
