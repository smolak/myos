...
describe("Todo actions", () => {
	...
	describe("createTodo", () => {
		...
		test("persists title and completed=0 in DB", async () => {
			...
			const row = db
				.query<{ title: string; completed: number }, [string]>(
	...
	describe("updateTodo", () => {
		test("updates the title", async () => {
			...
			const row = db
				.query<{ title: string }, [string]>("SELECT title FROM todos WHERE id = ?")
				.get(id);
	...
	describe("completeTodo", () => {
		test("marks the todo as completed", async () => {
			...
			expect(result.success).toBe(true);
			const row = db
				.query<{ completed: number }, [string]>("SELECT completed FROM todos WHERE id = ?")
				.get(id);
