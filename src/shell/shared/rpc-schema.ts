...
export interface AppRPCSchema extends ElectrobunRPCSchema {
	bun: {
		requests: {
			...
			"pomodoro:get-settings": { params: Record<string, never>; response: { workDurationMinutes: number; breakDurationMinutes: number } };
			"pomodoro:update-settings": { params: { workDurationMinutes?: number; breakDurationMinutes?: number }; response: { success: boolean } };
			...
			"weather:get-settings": { params: Record<string, never>; response: { apiKey: string; location: string; units: "metric" | "imperial" } };
			"weather:update-settings": { params: { apiKey?: string; location?: string; units?: "metric" | "imperial" }; response: { success: boolean } };
